...
**[E:dump-terrain]** — a serpentine-dumped pill's true square shows up at its next pickup: the `FF 0n` names the pill outright, and the picker's centre square rides the same record ([E:centre-square], [E:centring]). Matching that square against the serpentine path from the death square gives the path position the pill really occupied, model-independently — every earlier unoccupied square was *skipped* by the real game, the observed one *accepted*. Over the sample log and a 2002 replay (md5 `05802109fccca307403b7d42a4da8f62`), 57 of 62 dumped pills resolve to an exact path square (3 ambiguous, 2 never picked up), the resolved positions are strictly increasing in pill-index order at every multi-pill dump — 0 violations, confirming lowest-index-first placement along a single never-backtracking search — and **no terrain was ever skipped**: all 5 skips were squares occupied by a pill or base. Terrain accepted: grass 24, road 27, rubble 3, **river 3**.

The river cases are the decisive ones, since the model had been refusing water. In the 2002 replay a tank died fording (death square 102,124, mid-river) carrying pills 0 and 6; they landed on the death square itself and the square north of it — the first two path positions, both river. The collector's tank centre entered each square exactly at the record carrying its pickup, and under the water-refusing predicate those two were the only pickups of the log's 153 to miss their modelled square, both pills having slid two path positions into the forest; the sample log carries the same signature in miniature, its one river-death dump being its only off-square pickup of 322. With river accepted, all 475 pickups across the two logs land exactly on their modelled square. What the search *refuses* is thinner ground: no observed dump has yet reached a building, boat, or deep sea before placing, so the refusals rest on gameplay experience — building and shot building (the impassable terrains) confidently, a boat less so — and deep sea is believed dumpable on the same authority. Whether a search reaching deep sea would prefer a later river square is likewise untested. `node tools/measure-dump-terrain.cjs`.
