...
The corpus run settles the rest. Over 443 logs: control 29,573 exact against 15 off-by-one (0.05%, the method's noise floor); 4,024 serpentine dumps of 6,121 pills, 5,536 resolved to exact path squares, and **0 order violations**. The terrains observed skipped are exactly the impassable set — **building 20, shot building 28, boat 2** — and nothing else, ever. Everything else is accepted, with counts: road 2,939, grass 1,760, forest 271, crater 223, rubble 152, **deep sea 60**, mined road 42, river 31, mined grass 24, swamp 12, mined forest 9, mined crater 7, mined swamp 1, mined rubble 1. Deep sea's 60 rests against 0 skips also rule out a river-over-deep-sea preference: a search reaching water takes it at once. The lone blemish is 4 resolutions landing on squares the model called *building* at dump time, against its 20 skips — 0.07% of resolutions, at the noise floor where ~3 false resolutions are expected, and doubly implausible as genuine rests because the observation is a tank centre tile, which cannot lie in standing building terrain; a terrain change riding the dump record itself (the snapshot is pre-record, the dump mid-record) is the other innocent reading, and the tool now quarantines such squares and prints every refused-terrain rest in detail for inspection. `node tools/measure-dump-terrain.cjs`.
