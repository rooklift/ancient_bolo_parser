...
**[E:shell-list-skew]** — measured by `tools/measure-shell-list-staleness.cjs`: for every pair of same-pill matched shells whose orbit step is unambiguous at both ends of one statement-to-statement transition, the difference of their orbit-step advances, bucketed by whether the pair shared a shell list at each end. Over three replays (the committed fixture plus two laggy corpus games, `022603.5` and `110702.1`; 108,361 pairs), the advance difference is **zero** in 99.78% of same-list/same-list pairs and 99.61% of cross-list/cross-list pairs — the pill-wide lockstep is a strong prior even across lists — but the nonzero tail runs to 10 steps (20 ticks) and concentrates two-to-fourfold in pairs whose list membership differs or churns between the two statements. The tally conditions on chains the matcher linked with a single surviving orbit state at both ends, so it *undercounts* skew: a badly skewed statement is exactly the one whose chain tends to break, and broken chains contribute no pairs; read the nonzero fractions as a floor.

Two hand-verified cases pin the phenomenon beyond matcher trust. In `022603.5` around 2:56 (records ~821–829), one record's direction-12 list is stated about four ticks (two updates) staler than the direction-13 lists *in the same record*: across the next ten-tick gap both direction-12 shells advance seven steps where five are possible, in perfect mutual lockstep, every statement an exact orbit point of the same pill — a whole list stale together, its neighbours on schedule. In `110702.1` around 45:45 (records ~44600–44615), the skew reaches ten steps against three-to-four for other lists of the same pill in the same transition. Consequences for playback: exact common-advance may be relied on within one list; across lists it is a prior, not an invariant; and while the sender's true simulation keeps one pill's shells in strict distance order (each advances one step per update pass), a skewed statement can misreport that order, so apparent overtakes between shells stated in different lists are not evidence of real ones.
