...
| `7T` | 3 | event at `XX YY`, usually an explosion, whatever set it off (a shell, a mine, a boat sailed over, a dying tank); `T` = new terrain, or `B` = no terrain change, `C` = LGM plants mine (no explosion), `D` = four-square superboom (craters the square and its E/S/SE neighbours, sparing water, bases and pillbox squares). A superboom also deals 4 **eventless** damage to any pillbox in its four squares — no `9n` is sent [E:superboom-pill]. Cratering, single or superboom, spares the ground under a grounded pillbox, dead or alive, and spares open water; a **boat** square is not spared — it craters, and then floods [E:crater-pill] [E:crater-water]. Crater *flooding* IS evented: water-adjacent craters become water via explicit terrain changes, 25–37 ticks (0.50–0.74 s) later. The causes: a tank sailing its boat over a boat square destroys that boat and sends `7 1` for it [E:boat-over-boat], a mine detonating and a dying tank's terminal crater are `7 3` (see `F9`), and playback that turns `7T` into shell impacts must sort them by what lay under the square |
| `8d` | 1 | unused |
...
| `F7` | 1 | tank lays mine |
| `F8` | 2+len | node id: Pascal string `player@node`; also sent on rename. A joining player's F8 has tank status `T=7` and is followed by established ring members restating their unchanged F8 ids; together those distinguish a slot admission from an isolated rename even when the former occupant's quit was lost. A nuBolo joiner also says so outright, with `FF F7` |
...
| `FF F7` | 3+3f | player join, seen only from nuBolo: the quit's layout, the joiner's upstream / self / downstream ring neighbours. It follows the joiner's `T=7` F8 in the same record [E:nubolo-join] |
...
nuBolo's machine names are addresses too. The machine half of a nuBolo player's `F8` node id is the machine's IPv4 address and UDP port as 12 hex digits, the same six bytes a quit record's address fields carry: `name@C0000201C350` is 192.0.2.1, port 50000. The parser does not use this to detect nuBolo, and the viewer's players panel shows the address in place of the hex. `tools/redact-addresses.cjs` patches only the game info, quit and join records, so redacting a nuBolo log needs these names replaced to match, through the name mapping [E:nubolo].
...
- **Dropped pills on tank death** are placed by an algorithm (a clockwise spiral outward from the death square), not by explicit events. Carried pills are placed lowest index first, each taking the next acceptable square of a single never-backtracking search. The spiral is the death square, then each ring of squares around it in turn, and every ring is walked the same way: starting due north and going clockwise all the way round — so the second ring begins directly north and ends on the square NNW of the death square, not the other way round. The map's mined border, its outermost ten rows and columns, is refused as well [E:dump-terrain]. The search accepts almost any square — river, deep sea and every mined variant included — refusing only a square already holding a pill or base and three terrains: building, shot building and boat [E:dump-terrain]. One exception: if the sender's man is out of the tank carrying a pill (status `C`) when it dies, the lowest-index carried pill is in his hands rather than in the tank, so it is not dumped; he goes on to plant it, and that plant arrives as an ordinary `FF 50` [E:man-carrying].
- **A quitting player's carried pills drop the same way** around the tank position the quit record itself states, with no events. **A player the ring drops in a netsplit drops his cargo the same way** around his last tank position, with no record of any kind, and does not get it back on rejoining: he drives back and picks his own pills up. The log cannot see the ring drop him, only his silence, which is a noisy proxy — one player was silent 15 s while the ring turned 124 records and still held his pill — so playback drops the cargo at a classified rejoin, and before that only after 16 s of silence while a hundred records came from the others. A leaving player's man drops the pill in his hands where he stands [E:quit-pills].
...
Almost everything that looks wrong in a log turns out to be a rule we had not worked out yet, so the bar for this section is high: the game itself has to be at fault; playback corrects the fault where it can and reproduces it where it changed the game.
...
- **A shell fired as its tank crosses a sector boundary is listed under the wrong direction for life.** The shell list's direction nibble is the tank's coarse facing from one shell update (two ticks) before the shot, while the `5d` nibble and the shell's velocity are from the shot itself; whenever the facing crossed a sector boundary inside that update, without exception in the pinned-bradian test, the shell flies the nibble's sector but is listed a sector off, and never re-listed. It is about 4% of the fires whose shell is seen at the muzzle, in both turn directions alike, across 378 of the 443 logs; the `FC` hit packet carries the true sector, so a reader trusting the list label loses every tank hit such a shell scores. Osterwald saw the disagreement and suspected a bug; the flight test settles which side is right. Playback takes the nibble as the shell's sector and keeps the list label only for matching restatements [E:shell-birth-sector].
- **About one chat message in 4,500 goes out with a zero length and a tail of buffer leavings.** Ten of 44,374 messages over both collections have a Pascal length byte of zero, and every one of the ten is followed in its packet by bytes that are not subpackets, while no legitimately empty message exists at all. Read as subpackets the leavings are terrain changes and explosions on deep sea, 79 to 195 squares from the sender, and three-shell lists at absurd positions; one painted a shot building into open water. Their first byte is always printable, and each tail is exactly as long as the one subpacket that byte would name — 3 bytes after a letter, 8 after a space — so the sending machine evidently sized the packet by walking its own buffer with the opcode table and copied what the leavings claimed. The fault is the sender's: one of the ten is the recorder's own outgoing message, and the other nine reached two different recorders at like rates, the junk inside the packet's length. Chat is otherwise well-formed, and shell lists genuinely follow a real message in about a thousand records, so playback ends the record only at a zero-length message and keeps the tail for inspection [E:empty-chat].
...
- **Rob Keogh ("Pins")**, whose working notes and questions flag several fields as unknown; where this document resolves one, it says so.
...
- **Our own empirical analysis**: a 120,840-record log from October 2002 (the "sample log"), a second sample log, and a private corpus of 446 logs (2001–2005, 13.4 million records, all Bolo 0.99.7), three of which are corrupt in some way and excluded from corpus-wide measurement, leaving 443. Corpus figures in FORMAT.notes are from the 443-log set unless an entry says 446, in which case it was run over the full set. A second private corpus of 587 logs supplied, among other things, ten games each logged on two machines at once — the only chance the format offers to see the same packets twice [E:two-recorders]. Those twenty logs are committed as `fixtures/pairs/`, named by date with `-A` and `-B` for the two recorders, after the same anonymisation as the sample log: player and machine names, chat and IP addresses substituted byte for byte, every other byte as recorded, so the two logs of a game still agree on every shared record.
- **Manual observation of original Bolo in an emulator**, for behaviour no log records directly, and a single-player log recorded there, `fixtures/emulator_solo`, whose controlled trials are read in FORMAT.notes.md under [E:emulator-log].
