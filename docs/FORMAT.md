...
- **A quitter's planted pills and bases apparently go to nobody when his only ally is dead.** A departing player's property passes to an ally, but seemingly only to one who holds a tank at that moment: over the corpus, pills handed to a live ally never fired at him (0 of 2,281), while pills whose lowest-index ally was dead or tankless at the quit went on to fire at that ally once he respawned (7 of 243, three replays), and a quitter with no live ally left pills that shot his dead ally 40 times. A dead player is still a member of the alliance, and the manual promises the alliance keeps the property, so this looks like Bolo searching its live tanks for an heir rather than its players. The sample of dead heirs is small, and whether a second, live ally inherits instead rests on two quits. Unlike the index fault, it changed the game: the orphaned pills were hostile to everyone until killed and re-planted. Playback reproduces it, because the corpus shows it happening; a disconnect presumably behaves the same, since the log records both as a quit [E:pill-target].
