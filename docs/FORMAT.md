...
| `FC dn` | 2 | shell (direction `d`) hits tank `n`. Sent by the machine **simulating the shell** — the firing tank's owner for a tank shell, the pill's target for a pill shell, whichever tank it strikes — so a tank hit by a pill aimed at it reports its own hit, a tank hit by another tank's shell does not, and a pill shell that strikes a tank other than its target is reported by the target, not the tank hit. No hit is reported by two machines [E:hit-reporter] |
| `FD`, `FE` | 1 | unused |
