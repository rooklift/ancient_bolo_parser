...
FORMAT.md's Terrain section carried from the start, untagged, the rules that shells pass over open ground and are stopped by building, forest, shot building and boat, that tanks are blocked by building and shot building, and that one hit fells a tree, one turns a building into a shot building and four more turn that to rubble. Those are rules of the game rather than of the log, so they now live in GAMEPLAY.md, and this entry reads them off the two fixture logs: `node tools/measure-terrain-hits.cjs fixtures`. Squares under a base are excluded throughout, since a base's square is road whatever lies beneath ([E:base-road]). Not run over the corpus.
...
**What stops a shell.** A `7T` impact that names a new terrain is a shell ending against something it changed. Leaving aside the craters, which are mines going off and the terminal craters of dying tanks ([E:death-tiers]) rather than blocking terrain, the fixtures hold 1,578 such impacts: forest to grass 536, building to shot building 398, shot building to rubble 445, boat to river 179, 18 that name the terrain already there (rubble 8, grass 5, shot building 4, river 1) and 2 (grass to swamp, road to river) where the model's ground is presumably stale. A `7B`, an explosion with the terrain unchanged, is a shell ending against something it did not alter: 1,249 of the 1,255 are on shot buildings and the other 6 on grass, unexplained. No `7B` ever lands on a building, so a building never absorbs a hit unchanged.

**What a tank drives on.** Of 130,540 live tank centre squares (tank position restatements without the dying bit, centred as in [E:centre-square]), none is a building or shot building. The rest: road 87,339, grass 29,815, forest 5,695, river 3,180, deep sea 1,877, rubble 1,522, crater 923, boat 183 and swamp 6, the water being tanks in boats, whose square the model has already reverted to river or deep sea ([E:boat]), and tanks fording. This is the set the death dump refuses ([E:dump-terrain]) minus the boat, which a tank can drive onto and consume.

**How many hits a shot building takes.** Each shot building is followed from the `7 8` that made it to the `7 6` that flattened it, counting the `7B` impacts on the square in between; a build or regrowth on the square in the meantime drops the life, and a square shot before logging began is never tracked. Of the 310 lives, 193 took exactly three unchanged hits, so rubble came on the fourth further hit, and none took fewer; 54 took four, 35 five, and the tail runs to fourteen. Splitting the lives by whether any one record carried more than one hit on the square: where every record carried one, 129 of 156 are at three; where some record carried two or three, 64 of 154. The bursts are real hits and not duplicated records: they arrive in the records of a pill's target, with as many `F4` fires in the same record, when several angry pillboxes fire into one wall at once and their target simulates and logs every shell ([E:pill-shell-migration]). Every life above three is in the sample log, `n20021018.2` (120,840 records): `040601.6` has 21 lives, all at exactly three. Two to look at: square 137,120 is shot at record 2,486 (2:57.16), takes two unchanged hits in record 2,490, three in 2,494 and falls in 2,498, all logged by player 1 under fire from two and three pills at once; square 129,112 is shot at record 6,636 by player 3, hit once by him at 6,657, then three times by player 1's tank at 7,142–7,150 and flattened by it at 7,170, one shell per record. The count is not kept per machine: in `040601.6` the three unchanged hits of a life are routinely split between the two players (square 134,132, records 7,796, 9,089 and 9,123), and collapsing the hits of one record into one hit only moves the mode to two. Whether the game counts the hits of one tick once, or a shot building's damage is something other than a count, the fixtures cannot say, and GAMEPLAY.md lists it among the open measurements.
