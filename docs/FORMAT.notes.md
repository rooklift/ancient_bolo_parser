...
Corpus figures are from the 443-log set unless an entry says 446, in which case it was run over the full set (see the Sources section of FORMAT.md); "the sample log" is the fixture `long_game`, and a "second corpus" of 587 logs is named where it was used.
...
- [`[E:sender-lockstep]`](#esender-lockstep-every-pill-one-sender-simulates-advances-in-lockstep) — every pill one sender simulates advances in lockstep
...
So the sequence byte is a **ring slot counter**, stepped by every node as the packet passes whether or not that node has anything to say, and a hole is a quiet slot. The figure the viewer once rated as loss is the share of the ring's turns on which the node had nothing to log: it rises with player count because more players means more of them parked or dead at any moment, and it reads as catastrophic while a game gathers because everyone is waiting. It has since been dropped from the net-conditions verdict, which now reads stall and cycle alone; the viewer no longer shows it, though `network_conditions` still returns it for the measurement tools. What follows stands as measured — the same numbers under their real name.

**No loss at all, and a second log is the only way to know.** The holder's scan of both corpora (`docs/corpus_runs/73d8573-recordings.txt`) found ten games logged on two machines at once; across them, of 38,318 missing slots in one log's holes inside the shared stretch, **none** is a record the other machine received.
...
**What the steps track, once the ramp is excluded.** The quiet-slot share rises with player count, more players meaning more of them parked or dead at any moment (it was read at the time as a ring gaining a hop per player, each hop another chance to drop a packet): medians 5.2% at two players, 6.6% at four, 7.4% at six. It also agreed at r = 0.69 with what was then an entirely separate reading of the same stream — the share of elapsed time in gaps over half a second, during which nothing arrived at all. That agreement was the two readings sharing a fault: both were counting idle games. Once the stall reading was gated on the slot counter the correlation fell to r = 0.025 over both collections ([E:idle-stall]), which is what a genuinely independent reading of the same stream looks like. Over settled play the corpus runs from 1.3% in the cleanest games to 49.5% in the worst, median 6.3%.
...
It earns its place in the verdict by predicting what a viewer can make of the stream: the share of shell observations the motion pipeline fails to chain forward tracks cycle p90 at rho = 0.76, against 0.41 for stall and 0.25 for loss — and the loss figure's correlation is mostly player count in disguise, collapsing to 0.02 within four-player logs while cycle keeps 0.71. (Those are the 443-log figures at the pipeline of the day; over both collections at `f4fe3b7` the four-player collapse holds at -0.02 for the quiet share with cycle keeping 0.64.) The poster child is a four-player log with 2.7% loss whose ring turned every 0.3 s: the corpus's worst shell interpolation on nearly its cleanest loss figure. Over both collections (1,020 logs, `docs/corpus_runs/9596cdc-agreement.txt`) the share of tank track segments the motion code cannot bridge tracks cycle at rho = 0.91 and stall at 0.48; partialling each out of the other leaves cycle at 0.89 and stall at 0.32, and among rings under 14 ticks the median shell-unchained rate is flat (0.19% to 0.43%) from no stall at all to over 25%. The stall figure there is the ungated one. At `f4fe3b7` the same row reads 0.63 with cycle unmoved at 0.91, the partials 0.85 for cycle and 0.23 for stall, and the fast-ring flatness is gone: held to rings under 14 ticks the gated stall stages 3.69 / 4.40 / 4.91 / 5.13 / 5.50% of segments unbridged from no stall through each band in turn ([E:idle-stall]). Stall's agreement with the pipeline is thus mostly cycle's; what stall measures on its own is the freeze a viewer sees whether or not the shells chain. Reproduces with `tools/measure-network-agreement.cjs`.

Scoring interleaved half-minute blocks of the settled span as though they were two separate games gives r = 0.88 on loss, 0.94 on stall and 0.99 on cycle time, so this is a property of a session rather than of whichever minute was sampled, and fair to state once for a whole game — which is what the viewer's header does, via `network_conditions` in `viewer/network.js`, the verdict being the worse of the stall and cycle bands, the quiet share computed but not rated. All of the above reproduces with `tools/measure-network-conditions.cjs`.

**Where the bands are cut.** Neither reading has a natural break: the pipeline's failure rates rise smoothly with the ring cycle, about half a point of unbridged segments per tick from 6 to 30. The cycle cuts, 10 / 15 / 21 ticks, are severity levels on that gradient — the ring's floor is 6 ticks, and by 10 the unbridged share has risen by half, by 15 doubled, by 21 tripled — placed to stage the outcomes and spread the corpus, since with no external standard for a good 2003 connection the words can only rank games against each other. They were first set at 14 / 19 / 26, which put 70% of the first collection's logs in "good" across a range over which the failure rate doubles. The stall cuts, 2 / 4 / 8%, are felt-time levels chosen by judgement (roughly one, two and five seconds frozen per minute, in silences of over half a second each), since the pipeline cannot anchor them; they were first set at 2 / 7 / 18, whose upper two sat far out in a reading with corpus quartiles of 0.1 / 0.7 / 2.9%, so that a game frozen a tenth of the time was not yet "awful". Lowering them moved 10% of verdicts, 50 logs into "bad" and 52 into "awful"; by the pipeline those logs read like the band they left (a median 4.7% of segments unbridged among those moved into "bad" against 4.9% for those staying "fair", 5.6% among those moved into "awful" against 7.3% for those staying "bad"), which is the reading flagging the freeze the pipeline cannot see, at a cost of 1.6 points of half-to-half agreement and four points of the band's rank correlation with segments unbridged, the per-band medians still staging in order. Both readings' cuts are in ticks and percent rather than corpus quantiles, so they stay put as the corpus grows. On both collections the bands split 36.5% good, 35.9% fair, 18.8% bad, 8.7% awful, halves agreeing 94.8% of the time; before the ring-turned gate of [E:idle-stall] the same cuts gave 32 / 34 / 20 / 13 (the first collection alone 46 / 31 / 16 / 7) and 92.5%. The band's rank correlation with segments unbridged was 0.78 at those pre-gate figures (0.82 at the earlier stall cuts, 0.67 at the earlier cycle cuts); the gated reading takes the same cuts to **0.85**, the best any combination in these notes records.
...
**The two populations are bimodal, but they touch.** `tools/measure-ring-stalls.cjs` buckets every silence over half a second by how far the counter moved. Over both collections (`docs/corpus_runs/f4fe3b7-ring-stalls.txt`, 1,020 logs, 31,792 silences):

| ring | step 0–1 | 2–3 | 4–6 | 7–15 | 16–31 | 32+ |
|---|---|---|---|---|---|---|
| 2 players | 4,228 | 375 | 156 | 2,375 | 1,535 | 1,464 |
| 3 players | 651 | 117 | 6 | 76 | 68 | 57 |
| 4 players | 9,582 | 1,164 | 121 | 571 | 2,099 | 2,277 |
| 5 players | 1,185 | 138 | 10 | 33 | 69 | 62 |
| 6 players | 1,425 | 170 | 5 | 119 | 193 | 89 |
| 7 players | 1,234 | 88 | 4 | 7 | 2 | 2 |
| 8 players | 19 | 6 | 1 | 2 | 6 | 1 |

64.1% of silences step by 3 or less and 34.9% step past 6; the 4-to-6 middle holds 303 of them, 0.95%. Two thirds of every silence over half a second in the corpus is a real freeze (20,472 of 31,792 once each ring's own threshold is applied), and a third is not. **The twenty-two committed fixtures had predicted an empty middle and were wrong** — their whole 4–6 column is zero — because they hold almost no parked games, so the fixture-scale separation was an artefact of a small and unrepresentative sample. What the corpus shows instead is a thin valley rather than a gap, and a silence within a slot or two of one ring's worth is genuinely ambiguous: a ring that limped round once, rather than one that clearly stopped or clearly ran. The counter cannot settle those, and no reading of the log can; there are 213 of them above the cut and a few hundred below it.

**What settles the gate is what it removes, not where it cuts.** Two figures, both from the corpus:

- the stall reading's correlation with the **quiet share** — which is the idleness measure — falls from **r = 0.463 to 0.025** (`f4fe3b7-conditions.txt` against `9596cdc-conditions.txt`). The old stall figure was half an idleness reading; the gated one is independent of it, which is the whole claim in one number.
- its rank agreement with the share of **tank track segments the pipeline cannot bridge** rises from **rho = 0.478 to 0.634** (`f4fe3b7-agreement.txt`). That row is the clean before-and-after: its quiet and cycle correlations are byte-identical across the two runs, so nothing but the stall reading moved. The shell and terminal rows shifted with the interpolation engine in the commits between and cannot be read the same way.

So the gate takes out idleness, and what it leaves predicts the damage a viewer will see better than what it started with — which is what a reading is for.
...
It is not a curiosity. Over the corpus the gate clears silence in **315 of the 1,020 logs and moves 86 verdicts**, nearly all downward in severity: 133 "awful" logs become 89 and 206 "bad" become 192. The stall distribution drops with them — p50 0.7% to 0.2%, p75 2.9% to 1.8%, p90 8.9% to 5.6% — while the maximum, 67.1%, does not move at all, which is the shape a reading has when what it loses is a contaminant rather than a share of everything. Half-to-half band agreement improves from 92.5% to 94.8%. The parked games are plain in the run's own table: one four-player game whose four tanks each state a single position across 20.9 minutes of silence, another whose three heard-from tanks do the same across 12.7.

**The cuts stay where they are, and now have an anchor they never had.** The stall cuts are felt-time levels rather than corpus quantiles, so a reading that got more honest does not move them: a game now over 8% really is frozen five seconds a minute. What changed is that the pipeline can see the reading at all. `measure-network-agreement.cjs` grew three sections and was re-run over the saved rows (`docs/corpus_runs/f4fe3b7-agreement-reprint.txt`; `--rows` reprints from a `--json` without re-reading the corpus, and every line of the original run reappears in it byte for byte):

- **the verdict band** against segments unbridged rises from rho 0.78 to **0.85**, the best any cuts in these notes record;
- **held to fast rings** — cycle under 14 ticks, where a slow ring cannot be the culprit — the gated stall stages across its own cuts instead of reading flat: from no stall through each band in turn the median segments-unbridged share runs **3.69 / 4.40 / 4.91 / 5.13 / 5.50%** and shells unchained **0.13 / 0.19 / 0.21 / 0.25 / 0.31%**, rising at every step. On 341 / 297 / 53 / 21 / 14 logs, so the top two bands are thin. Ungated the same table read flat (0.19% to 0.43% of shells from no stall to over 25%), which is why the cuts were said to have no anchor. They were not placed on this and are not moved by it; it is the first evidence they sit anywhere sensible;
- against those, one figure moved the other way. Stall's **partial** — what it knows that the cycle does not — falls from 0.32 to **0.23** on segments unbridged. The gate raised stall's raw agreement by making it agree with the cycle reading (r 0.493 to 0.625), so more of what it now knows is knowledge the verdict already had. Stall keeps its place on the ground it always had, that a freeze is plain to a viewer whether or not the shells chain, and not on a claim to independence it does not have.

**The gate can only clear a silence, never convict one.** The counter is 7 bits, so a silence past 128 slots — 175 to 400 ticks at the slot rates these logs run at — can wrap and alias a large step down into the frozen range. That misreads an idle stretch as a freeze, which is exactly what the reading did for every silence before the gate, so the failure is the old behaviour rather than a new one. A stopped ring has no large step to alias from, so a real freeze is never cleared.

**A note on how this was arrived at.** The gate was written and its case argued on the twenty-two committed fixtures, which said it was a near no-op — a 0.12-point change to one log and no rating moved. The corpus says otherwise on both counts: it is 86 verdicts, and the clean separation the fixtures showed does not exist at scale. The fixtures were not wrong about any log in them; they were an unrepresentative sample, holding almost no games anybody walked away from. Anything measured on them alone about how often a thing happens should be read as a lower bound on the corpus, not an estimate of it.
...
The quantiser itself is recoverable once an orbit supplies the internal 1/16-pixel coordinates: each byte is `(next_internal - previous_internal) >> 4`, with a signed arithmetic shift. It reproduces 18,873 of 18,875 adjacent uniquely resolved pill-shell pairs in `long_game` and 1,720 of 1,721 in that replay, including roughly 18,000 negative components; the three contradictions are consistent with the provisional shell identity/source being wrong. For the final pair above, the internal coordinates are `(-1857,-801)` and `(-1771,-690)`, whose difference `(86,111) >> 4` is exactly the logged `(5,6)`. This makes the raw offset chain useful evidence rather than mere error: candidate orbit states for adjacent members can be retained only when their exact internal coordinates reproduce the byte pair.

Those tallies were a one-off count made when the constraint went into the viewer and were never scripted. `tools/measure-shell-offset-quantiser.cjs` re-derives them (`docs/corpus_runs/a9259de-quantiser.txt`) with one change of method: the viewer has since applied the shift rule itself as a pruning constraint on orbit hypotheses, which would make a count circular, so the tool builds with that pruning switched off and conditions only on adjacent same-pill members that position, timing and lockstep evidence pinned to one state each. Over the 24 committed fixtures that is 81,769 pairs, and the shift rule reproduces the logged byte pair on 81,754 of them (99.98%; 13,863 of 13,863 in `long_game`). The two alternatives fail on exactly the pairs that separate them: subtracting the two rendered pixels predicts different bytes on 57,545 pairs, and the log sides with the shift on 57,533 of those and with the pixels on none; a truncating division differs on 54,178 pairs, likewise none. The 15 pairs the shift rule misses are off by one or two in a component and match neither alternative either, and three of them are one pair of shells restated three times with the same one-pixel miss, which is what a wrongly resolved state for one member looks like rather than a different quantiser.

The same run grades the bound itself. For every member whose whole chain back to the head is same-pill and pinned to one state (80,434 members), the exact position minus the reconstructed one is never negative and never exceeds the member's index on either axis: 0 of 47,564 first members, 21,998 second and 10,872 third are outside `[0, i]`. The full miss is ordinary, not a corner case: 890 of the 10,872 third members (8.2%) are 3 pixels out on at least one axis, and 31 are 3 out on both, so a consumer that treats a third member as exact is a tile-fifth wrong that often.
...
### [E:sender-lockstep] — every pill one sender simulates advances in lockstep

The lockstep is the *sender's*, not one pill's. A client simulates the shells of every pill firing at its tank, and steps them all in one update pass, so between two of its records every pill it carries should advance by the same count. `tools/measure-cross-pill-agreement.cjs` replicates the roster vote's own gates (three pinned statements, a margin of two) per pill and asks, over each sender record pair, whether the pills agree. Over the 443-log corpus (`docs/corpus_runs/cb1fd5d-cross-pill.txt`; 1,858,801 sender pairs with a pinned roster at both ends, 262,259 carrying two or more pills) the answer is unanimous: in all 10,434 pairs where two pills each passed their own election they elected the same advance, and in the 98,672 cases where one pill elected beside a pill too sparse or too ambiguous to vote, 99.77% of the sparse pill's 100,819 pinned statements land exactly on a pinned statement of the next record at the rich pill's advance, where only 22.9% could be landed by any other advance in the window. Summing the per-advance scores over all a sender's pills under the same gates elects 77,948 pairs no single pill could (a 14.5% gain on the 536,479 per-pill elections) and never contradicts a per-pill winner. The pill-wide common advance is therefore one number per sender transition across every pill the sender simulates, and a pill's statements are evidence for its stream-mates from other pills.

The viewer does not act on it. Lending the sender's advance to every pill of the sender that could not elect its own, at both of the roster vote's sites, was built and measured on the corpus (`8a54fd8`, `8212c0c`) and reverted: the vouched-link column rose by a tenth of its range, but the drawn picture was a wash -- a few dozen scenes better, a few dozen worse, across 443 logs -- for a few percent of build time and a chain broken for good wherever a lent advance vetoed a link nothing replaced. `docs/interpolation_tests_corpus.md` has the account.
...
The sender of an `F4` is the pill's target, and the target is the nearest hostile tank. Over the 443-log corpus (1,162,752 fires; the holder's run at the tool's commit is archived as `docs/corpus_runs/0550a08-target.txt`), 102,929 fires arrive with two or more hostile tanks inside a pill shell's 8.5-tile flight; in 96,049 of those (93.3%) the sender's tank is the nearest of them, against 49.6% if the simulating machine were drawn at random. Of the 6,880 exceptions, 4,449 lose by 8 px or less and only 24 by more than 64 — stale restatements, since positions are read from each player's latest packet — and in 2,260 the nearer tank was hidden in forest. Those are not losses: ranked among *visible* hostile tanks only, the sender is nearest in 98,308 (95.5%), so all but one of the hidden-rival cases resolve, and the sender's own tank is hidden at the moment of a fire in only 4,329 of 1,156,489 corpus fires (0.4%; 17 of 13,009 on the sample log). A pill does not target a tank hidden in forest; the residue is the size the hidden bit's own staleness would produce.
...
A tankless heir losing out looks like a bug, and a bug could as easily hand the pills to an enemy, so the next run tallied each quit's orphaned-pill fires by target against candidate heirs (archived as `docs/corpus_runs/0550a08-target.txt`; a real heir is never fired at).
...
`fixtures/emulator_solo` is a 17.7-minute strict game the owner played alone in a Macintosh emulator in September 2026, recorded for the experiments this file had left open: one machine, one tank, 7,656 records, no ring and so no lost or late record, every shell the tank's own or a pillbox's aimed at it, and the ground truth typed into the chat ("so I have 30+30", "So I have 40+21", "now restored to full"). `node tools/measure-emulator-log.cjs` prints every reading below (archived as `docs/corpus_runs/48c85e5-emulator.txt`), and the ammo integration of [E:death-tiers] agrees with every chat line, the first direct check that method has had.
...
**Walls.** Fourteen buildings followed from `7 8` to `7 6` with one tank the only shooter: thirteen took exactly three `7B` between, one shell each, 12–14 ticks apart. The fourteenth, (118,130), took five, and the tank never shot it: all seven hits are pillbox 7's shells, born into the wall beside it and logged in the record of each fire, three at the rested pace at 3:27–3:31 and four at 15:37–15:39, the last three 8 ticks apart with the pill angry. One case with two confounds: the two early hits may have been dropped over the gap (twenty other squares shot and six tank deaths in between, so a time-out, a bounded table of damaged squares and a reset on death are all live), or the angry fire may have been miscounted, which is where the corpus's tails of up to seventeen `7B` come from. Against the second, (137,125) fell under pillbox 8's angry fire on the normal count, though it logged its `7 6` twice, 1 tick apart, from a double fire, the repeat-announcement shape of [E:terrain-hits] on one machine. Two other walls, (125,118) and (118,125), were left for six and twelve minutes after only their `7 8` and then took exactly three `7B` and the rubble, which every reading predicts. Two `7 8` on one square 2 ticks apart (a tank shell and a pillbox shell arriving together) and two `7 6` 1 tick apart both occur on this one machine, so the repeat announcements of [E:terrain-hits] are not only a cross-machine staleness.
...
**The second ring starts due north.** Osterwald's notes give the first ring exactly — due north, then clockwise round to the NW corner — and playback had extended it outward by starting every ring one square east of its NW corner, which is the same square as due north only on the first ring. Nothing above tested that: no dump in the fixtures of the time or the paired logs ever reaches a tenth path square. A 2001 replay, committed since as `fixtures/pill_dump`, does. A tank died carrying six pills on 51,151, beside a wall of buildings to its east: four pills took path squares 0, 1, 4 and 6 of the first ring (the two east-side squares being shot buildings, the NW two already holding an earlier dump's pills), and the remaining two spilled into the second ring. Under the east-of-NW-corner order the fifth pill would sit on 50,149, the NNW square, and the sixth on 51,149 due north; the pickups put the fifth on 51,149 and the sixth on 53,153, on the far side of the five building squares — the only two of the log's 147 pickups off their modelled square, and 50,149 a road square the method had recorded as skipped. With each ring starting due north and finishing with the top-edge squares west of north (empty on the first ring; on the second, the NNW square alone, last), the fifth pill is due north, the sixth is at 53,153, and all 147 pickups land exactly. A tank standing on a path square was considered and refuted by the same log: a bystander's tank centre did sit on the skipped 50,149 at the death tick, but 25 s earlier another dump had put a pill directly under a stationary tank, which picked it up on the next record, and later a tank on the death square itself took nothing away from the dump.

The corpus then put the order to the test it had never had. Over 1,030 logs (8,030 dumps of 12,574 pills counting the quits and netsplits below, 11,142 squares observed via pickup), ten dumps reach a tenth path square, and five of them can tell the two orders apart — `20010306~c4c704`, `20010316~ba883a` (the fixture), `20010920~4975a6`, `20020201~73cf23` and `20020209~e66152`, five different games over a year. Under the old order every one of the five shows a passable square skipped at exactly path position 9, the NNW square; under the north-first order all five are clean, and the other five second-ring dumps are consistent with both. Scored side by side on the same evidence, order violations against passable squares skipped: north-first 0 and 0 (outside the two cases below, which no ring order touches); east of the NW corner 0 and 7; one west of the NE corner, which is what the old comment actually said, 2 and 72; the NW corner itself 10 and 2,316; nearest by Euclidean distance, clockwise from north within a distance, 331 and 1,635. Rings beyond the second are still the pattern continued, and no dump in the corpus reaches a third. `node tools/measure-dump-terrain.cjs` reports a passable-terrain skip or an order violation loudly should one turn up.

**The mined border is refused.** One dump in the corpus (`20040926~ad0138`) is a tank in a boat dying at 9,133, in deep sea nine squares from the west edge of the world, carrying five pills. Every pill was picked up on x = 10 or 11 — path squares 2, 3, 4, 10 and 11 — with the death square and every other path square at x = 8 or 9 refused, seven in all, deep sea like the squares taken. Deep sea itself is accepted 93 times elsewhere, so it is the edge, and the edge is the map's mined border (owner: the game rings the map with indestructible sea mines), which the boat had just run onto. The border's width comes from the tanks: over the corpus, every tank whose centre reached the tenth square from any edge — x or y of 9, or of 246 — died within two seconds, 9 of 9, all in boats, on all four edges (west four, north, east and south one each), logged as `F9` code 3 like a sinking; tanks on the eleventh square lived, this very boat sitting on x = 10 for twenty seconds afterwards collecting the pills. So the mines occupy the outermost ten rows and columns on every side, and a dump refuses them, x = 9 out and x = 10 in, on the same footing as the pill or base it will not land on: a dumped pill clears an ordinary mine ([E:dump-mine]), and these it cannot. Playback's `DUMP_EDGE`; the tool tallies these skips as "at the map edge" and shouts should a pill ever rest inside the band.
...
The corpus has 429 quits by a player the model holds as carrying. Sixteen of those dumps were picked up later, and every pill of them lies within the first two rings of the quitter's tank square, one eight-pill dump (`20010117~0b3e38`) running into the second ring exactly as a death dump does. Of the 413 never picked up, 409 come inside the last minute of their log — the recorder or his opponents leaving at the end — so the silence is the log ending, not the pills vanishing.

Which tank position the pills fall around was a question the two original cases could not ask: their quit records carried no position. Two corpus quits do, and they disagree. In `20020124~8f741c` a tank restated 130,140, sent nothing for 2.4 s, and quit with a record stating 132,140; its one pill was picked up on 132,140, and playback's habit of dumping at the pre-record position had it two squares off, its twelve skipped squares the only run of passable skips the dump tool had ever seen on a death or quit. In `20021015.1~cc786e` a player went silent at 12:50, sent a bare node record at 17:31, and quit at 18:02 with a record stating 153,164 — pixel offsets 0,0, 56 squares from his last real position and five minutes after it; his pill was picked up on 115,108, that last real position. So the quit record's own position is where the tank is; the ghost's is not, but the ghost is a netsplit, and the next paragraph has already dropped his cargo where it was found.

**A netsplit drops the cargo too.** The one death dump in the corpus whose pill was picked up far away (`20011024~790bbe`, 26 squares) is not a dump at all. The player had picked the pill up at 122,135, driven on to 123,137, and gone silent; four minutes later he sent a join-type node record, appeared at a start square in a boat, and sank, and playback dumped "his" pill there. It was picked up on 123,137, his last square before the silence. The corpus has 14 classified joins by a slot playback held as carrying (13 the same name reconnecting, one a stranger), and every one of their 22 pills was next picked up within one square of the slot's last tank centre before the silence, mostly by the returning player himself, driving back for them. The 24 pickups of a pill playback held as carried — the only such cases in 70,153 pickups — are the same thing seen from the other side: 18 are the returning player collecting his own dropped pills, and 6 are other players taking them during the silence, the earliest 653 ticks (13 s) after the carrier's last record, seven seconds before he rejoined (`20020222~835dab`). So a player the ring drops leaves his cargo around his last tank position, tank-death style, and does not get it back.

When the ring drops him the log cannot say: no record marks it, and a silence in one recorder's log is a noisy proxy, since packets the recorder missed are not packets the ring missed. Measured over the corpus, a player's silence while the ring turned (records from the others in the gap) reaches 452 ticks at 156 records and 759 ticks at 124 records (`20021019.5~4697b2`, whose player then planted the pill he still carried 50 s later), against the 653 ticks and 103 records at which split cargo was first seen taken. No threshold on the silence alone separates those; ring-wide stalls of up to 36 s, everyone silent, resume normally. Playback therefore drops the cargo at a classified rejoin, always, at the slot's last tank position — the 14 cases above, exact — and before that only after 16 s of silence during which at least 100 live records came from the others, which no player in the corpus survived carrying; the six pickups during a silence then land on a pill playback still holds carried, which the pickup itself corrects, whereas a false drop would leave a later plant with nothing to plant. With the spiral, the edge, the quit position and the split together, the corpus reads: 8,030 dumps, 11,142 pill squares observed, 0 order violations, 0 passable squares skipped, and of 70,153 pickups 70,117 on the modelled square, 32 one square off (the control's own off-by-one count, the method's noise floor) and one of a pill still held carried, the 13-second case above. `node tools/measure-dump-terrain.cjs` treats these drops as dumps like any other, and the quit rule needs no ghost clause: a ghost's quit finds nothing to dump. One of the fourteen adds a detail: in `20011106` the dropped player's man was out of the tank, standing on 113,113 with the pill in his hands, beside a tank whose centre square was a base; the pill was picked up on 113,113, the man's square, not on the spiral's next free square from the tank. A leaving player's man drops his pill where he stands, and the tank's cargo spirals from the tank — the same split the tank death makes ([E:man-carrying]), except that the man is gone too and the pill comes to ground.
...
The terrain rules in GAMEPLAY.md, read off the logs: which ground a shell flies over and which stops it, which blocks a tank, and how many hits fell a tree, shoot a building and flatten a shot building. `node tools/measure-terrain-hits.cjs`; the corpus run over the 443 logs is `docs/corpus_runs/bb3f4c5-terrain.txt`. Squares under a base are excluded throughout, since a base's square is road whatever lies beneath ([E:base-road]).

**What a shell flies over.** An `FB` is the terminal point of a shell that flew its full range ([E:shell-fall-terminal]), so the ground under it is ground a shell crosses. Of 613,281 corpus falls, all but 37 end on road (386,071), grass (203,295), rubble (9,533), crater (8,558), deep sea (3,436), river (1,790) or swamp (561); the 23 on forest and 14 on building, one in 16,000, are the model's ground a record behind an event rather than shells crossing trees. On the fixtures alone the figure is 9,940 of 9,940.

**What stops a shell.** A `7T` impact that names a new terrain other than a crater is a shell ending against something it changed: forest to grass 93,355, building to shot building 25,442, shot building to rubble 24,503 and boat to river 3,381. Beside those, 1,485 name the terrain already there (grass 849, rubble 424, shot building 207, river 5): a second shell reaching the square before its client had applied the first shell's change, so both fell the tree, both finish the shot building, both sink the boat, and the second announces the change again. In 13 of the 14 fixture cases the previous event on the square is the identical change, 0–14 ticks earlier, from the same sender or another; the fourteenth is a `7 7` on grass with no event before it, a tree felled by a dying tank's eventless clearance ([E:forest-circle]). And 28 (crater to grass 9, road to river 5, road to grass 4, crater to shot building 2, swamp to river 2, crater to river 1) read as model error. The 5 grass to swamp are not: a shell fired from a boat at the shore turns the grass it lands on into swamp (owner), and the one fixture case, `long_game` record 79,100, is a tank in a boat shooting the square beside it.

A shell landing on a shot building normally logs a `7B`, the explosion with no terrain change: 75,382 of the 75,447 `7B`s are on shot buildings, the others grass 53, swamp 8, rubble 2, crater 1 and building 1, so a building absorbs a hit unchanged once in 25,443. The 207 `7 8`s on a square already shot are the same thing one step earlier, a hit announced as a change by a client that still held the square as a building: each of the 4 fixture cases arrives exactly one record, 12–13 ticks, after the `7 8` that shot it, three of them from the same sender, one from another. Such a repeat restarts the wall's count (below).

The craters are kept apart because a `7 3` is not simply a shell impact: on a mined square it is the mine going off, under a tank or under a shell, and on an unmined one a dying tank's terminal crater ([E:death-tiers]). The corpus has 16,804: mined road 4,627, road 4,427, grass 4,331, mined grass 2,133, crater 282, rubble 206, mined forest 200, forest 196, mined crater 148, river 128, mined rubble 59, boat 45, swamp 14, mined swamp 6 and deep sea 2.

**What a tank drives on.** Of 8,813,096 live tank centre squares (tank position restatements without the dying bit, centred as in [E:centre-square]), 43 are a building or shot building, one in 200,000 and presumably the model a record behind, and the rest road 5,108,274, grass 2,517,324, forest 608,732, deep sea 384,898, river 66,329, crater 59,796, rubble 58,313, swamp 5,428 and boat 3,959, the water being tanks in boats, whose square the model has already reverted to river or deep sea ([E:boat]), and tanks fording. On the fixtures it is 0 of 130,540. This is the set the death dump refuses ([E:dump-terrain]) minus the boat, which a tank can drive onto and consume.

**How many hits a shot building takes.** Each shot building is followed from the `7 8` that made it to the `7 6` that flattened it, counting the `7B` impacts on the square in between; a build or regrowth on the square in the meantime drops the life, and a square shot before logging began is never tracked. Of 17,894 corpus lives, 13,866 took exactly three unchanged hits, so rubble came on the fourth further hit as the owner's rule says, and none took fewer; 2,674 took four, 809 five, and the tail runs to seventeen. The excess sits with bursts: among the 12,078 lives whose every record carried one hit on the square, 11,208 are at three (93%), while among the 5,816 where some record carried two or three, 2,658 are (46%). The bursts are not duplicated records: they arrive in the records of a pill's target, which simulates and logs the pill's shells ([E:pill-shell-migration]), and each record carries exactly as many `F4` fires from that one pill as it carries `7B` hits on the wall, two or three a record from a fully angry pill. Whether one pill can really land two or three shells inside the 12 ticks of one record, or the game logs one hit more than once, is open. A repeated `7 8` restarts the wall's count. The corpus has 167 lives with a repeat, and in 139 of them it came before any `7B`, where a restart and no restart read alike. The 28 with hits before the repeat decide it: after the repeat they took three unchanged hits in 18 (before/after 1/3 in 16, 2/3 and 3/3 in one each), four or more in the other 10, and never fewer, while a whole life of three (1/2, 2/1 or 3/0), which is what no restart would give, never occurs. So every client takes a `7 8` as "freshly shot", whatever it held, and the wall's damage is state the events carry rather than a count each client keeps. This accounts for the lives with a repeat that run past three, 82 of the 167, and none of the burst excess, which has no repeat in it.

On the fixtures the tail is entirely in the sample log, `long_game` (120,840 records), where 117 of 289 lives go past three; `fast_ring` has 21 lives, all at exactly three. Two to look at: square 137,120 is shot at record 2,486 (2:57.16), takes two unchanged hits in record 2,490, three in 2,494 and falls in 2,498, all logged by player 1, the target of a single angry pill (index 13) that fires two or three times a record; square 129,112 is shot at record 6,636 by player 3, hit once by him at 6,657, then three times by player 1's tank at 7,142–7,150 and flattened by it at 7,170, one shell per record. The count is not kept per machine: in `fast_ring` the three unchanged hits of a life are routinely split between the two players (square 134,132, records 7,796, 9,089 and 9,123), and collapsing the hits of one record into one hit only moves the mode to two.
