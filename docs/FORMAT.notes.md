...
Scoring interleaved half-minute blocks of the settled span as though they were two separate games gives r = 0.88 on loss, 0.94 on stall and 0.99 on cycle time, so this is a property of a session rather than of whichever minute was sampled, and fair to state once for a whole game — which is what the viewer's header does, via `network_conditions` in `viewer/network.js`, the verdict being the worse of the stall and cycle bands, the quiet share computed but not rated. Those two bands place the corpus at 69.7% good, 20.4% fair, 7.9% bad and 2.0% awful, the two halves of a log agreeing on the band 96.8% of the time (`docs/corpus_runs/dcc6491-conditions.txt`). All of the above reproduces with `tools/measure-network-conditions.cjs`.
