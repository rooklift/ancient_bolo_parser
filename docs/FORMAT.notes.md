...
**The cuts stay where they are.** The stall cuts are felt-time levels rather than corpus quantiles, so a reading that got more honest does not move them: a game now over 8% really is frozen five seconds a minute. They still spread the corpus, the pipeline medians still stage in order down every column of the agreement run, and the band agreement went up rather than down. Three figures elsewhere in these notes still predate the gate because no tool prints them — the partial correlations, the band's rank correlation with segments unbridged (0.78), and the flatness of stall within rings under 14 ticks — and want the agreement run's `--json` rows to recompute.
