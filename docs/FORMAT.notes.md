...
**The map edge is refused.** One dump in the corpus (`20040926~ad0138`) is a tank in a boat dying at 9,133, in deep sea eight squares from the west edge of the world, carrying five pills. Every pill was picked up on x = 10 or 11 — path squares 2, 3, 4, 10 and 11 — with the death square and every other path square at x = 8 or 9 refused, seven in all, deep sea like the squares taken. Deep sea itself is accepted 93 times elsewhere, so it is the edge: a dump refuses the map's outermost ten rows and columns, x = 9 out and x = 10 in. Only the west edge has a witness; no pill of any kind, evented or dumped, rests within twelve squares of any edge anywhere else in the corpus, and the other three edges are assumed to mirror the west. Playback's `DUMP_EDGE`; the tool tallies these skips as "at the map edge" and shouts should a pill ever rest inside the band.
