...
The terrain rules in GAMEPLAY.md, read off the logs: which ground a shell flies over and which stops it, which blocks a tank, and how many hits fell a tree, shoot a building and flatten a shot building. `node tools/measure-terrain-hits.cjs`; the corpus run over the 443 logs is `docs/corpus_runs/ea8a8be-terrain.txt`, made before the craters were split out of the first table, so there they appear as `-> crater` rows. Squares under a base are excluded throughout, since a base's square is road whatever lies beneath ([E:base-road]).
...
**What stops a shell.** A `7T` impact that names a new terrain other than a crater is a shell ending against something it changed: forest to grass 93,355, building to shot building 25,442, shot building to rubble 24,503 and boat to river 3,381. Beside those, 1,485 name the terrain already there (grass 849, rubble 424, shot building 207, river 5), the model having got there first, and 33 (crater to grass 9, grass to swamp 5, road to river 5, road to grass 4, crater to shot building 2, swamp to river 2, crater to river 1) read as model error. The craters are kept apart because a `7 3` is not simply a shell impact: on a mined square it is the mine going off, under a tank or under a shell, and on an unmined one a dying tank's terminal crater ([E:death-tiers]); the corpus has 16,804 of them, road 9,054, grass 6,464, crater 430, forest 396, rubble 265, river 128, boat 45, swamp 20 and deep sea 2, and on the fixtures, where the tool now keeps the mine bit, they divide as mined road 71 against road 71 and mined grass 20 against grass 64. A `7B`, an explosion with the terrain unchanged, is a shell ending against something it did not alter: 75,382 of the 75,447 are on shot buildings; the others are grass 53, swamp 8, rubble 2, crater 1 and building 1, so a building absorbs a hit unchanged once in 25,443.
