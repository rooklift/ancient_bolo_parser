...
A shell landing on a shot building normally logs a `7B`, the explosion with no terrain change: 75,382 of the 75,447 `7B`s are on shot buildings, the others grass 53, swamp 8, rubble 2, crater 1 and building 1, so a building absorbs a hit unchanged once in 25,443. The 207 `7 8`s on a square already shot are the same thing one step earlier, a hit announced as a change by a client that still held the square as a building: each of the 4 fixture cases arrives exactly one record, 12–13 ticks, after the `7 8` that shot it, three of them from the same sender, one from another. Whether such a repeat restarts the wall's count is the question the tool's last table asks (below).
