...
**Where the bands are cut.** Neither reading has a natural break: the pipeline's failure rates rise smoothly with the ring cycle, about half a point of unbridged segments per tick from 6 to 30. The cycle cuts, 10 / 15 / 21 ticks, are severity levels on that gradient — the ring's floor is 6 ticks, and by 10 the unbridged share has risen by half, by 15 doubled, by 21 tripled — placed to stage the outcomes and spread the corpus, since with no external standard for a good 2003 connection the words can only rank games against each other. They were first set at 14 / 19 / 26, which put 70% of Nemokrad's logs in "good" across a range over which the failure rate doubles. The stall cuts, 2 / 7 / 18%, are felt-time levels chosen by judgement (roughly one, four and eleven seconds frozen per minute), since the pipeline cannot anchor them. Both are in ticks and percent rather than corpus quantiles, so they stay put as the corpus grows. On both collections the bands split 32% good, 39% fair, 20% bad, 8% awful (Nemokrad's alone 46 / 36 / 15 / 4), halves agreeing 94% of the time, and the band's rank correlation with segments unbridged is 0.82 against 0.67 at the old cuts.
