...
**What stops a shell.** A `7T` impact that names a new terrain other than a crater is a shell ending against something it changed: forest to grass 93,355, building to shot building 25,442, shot building to rubble 24,503 and boat to river 3,381. Beside those, 1,485 name the terrain already there (grass 849, rubble 424, shot building 207, river 5), the model having got there first, and 28 (crater to grass 9, road to river 5, road to grass 4, crater to shot building 2, swamp to river 2, crater to river 1) read as model error. The 5 grass to swamp are not: a shell fired from a boat at the shore turns the grass it lands on into swamp (owner), and the one fixture case, `n20021018.2` record 79,100, is a tank in a boat shooting the square beside it.

A shell landing on a shot building normally logs a `7B`, the explosion with no terrain change: 75,382 of the 75,447 `7B`s are on shot buildings, the others grass 53, swamp 8, rubble 2, crater 1 and building 1, so a building absorbs a hit unchanged once in 25,443. The 207 `7 8`s on a square already shot are the same hit announced as a change by a client that still held the square as a building: each of the 4 fixture cases arrives exactly one record, 12–13 ticks, after the `7 8` that shot it, three of them from the same sender, one from another. Whether such a repeat restarts the wall's count is the question the tool's last table asks (below).
