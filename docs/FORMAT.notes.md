...
The viewer's roster votes act on it: where one pill of a sender elects an advance, or where no pill can but their pooled statements do, the advance is lent to every other pill the sender simulates (`enforce_roster_lockstep_candidates`, `build_pill_lockstep_reference`); the fixture account is in `docs/interpolation_tests.md`.
