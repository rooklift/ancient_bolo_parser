...
**Walls.** Fourteen buildings followed from `7 8` to `7 6` with one tank the only shooter: thirteen took exactly three `7B` between, one shell each, 12–14 ticks apart. The fourteenth took five: two `7B` at 3:29 and 3:31, then three more and the rubble at 15:37–15:39, so the two hidden hits were not counted after the gap. That is one case, and it does not say why: twenty other squares were shot in between and the tank died six times, so a time-out, a bounded table of damaged squares and a reset on death are all live, and the corpus's long tails, which come from angry pillboxes within seconds, fit none of them. Two other walls, (125,118) and (118,125), were left for six and twelve minutes after only their `7 8` and then took exactly three `7B` and the rubble, which every reading predicts. Two `7 8` on one square 2 ticks apart (a tank shell and a pillbox shell arriving together) and two `7 6` 1 tick apart both occur on this one machine, so the repeat announcements of [E:terrain-hits] are not only a cross-machine staleness.
