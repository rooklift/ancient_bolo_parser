...
It earns its place in the verdict by predicting what a viewer can make of the stream: the share of shell observations the motion pipeline fails to chain forward tracks cycle p90 at rho = 0.76, against 0.41 for stall and 0.25 for loss — and the loss figure's correlation is mostly player count in disguise, collapsing to 0.02 within four-player logs while cycle keeps 0.71. The poster child is a four-player log with 2.7% loss whose ring turned every 0.3 s: the corpus's worst shell interpolation on nearly its cleanest loss figure. Over both collections (1,020 logs, `docs/corpus_runs/9596cdc-agreement.txt`) the share of tank track segments the motion code cannot bridge tracks cycle at rho = 0.91 and stall at 0.48; partialling each out of the other leaves cycle at 0.89 and stall at 0.32, and among rings under 14 ticks the median shell-unchained rate is flat (0.19% to 0.43%) from no stall at all to over 25%. Stall's agreement with the pipeline is thus mostly cycle's; what stall measures on its own is the freeze a viewer sees whether or not the shells chain. Reproduces with `tools/measure-network-agreement.cjs`.
