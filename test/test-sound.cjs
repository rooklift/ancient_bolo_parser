...
assert.ok(audios.length <= 9, "simultaneous copies of each sound are bounded");
...
// Each trigger gets its own bounded pitch variation, pooled voices included.
{
	...
	pitches.push(...Array(11).fill(0.5));
	...
	let burst = Array.from({ length: 8 }, (_, i) => ({ ...shot, x: 50.5 + i }));
	pitched.advance(burst, 0, 10, 1, -1, () => listener);
	assert.equal(voices.length, 8, "the pool grows to eight");
	for (let audio of voices.slice(2)) { audio.restarts = 0; counting(audio); }
	assert.deepEqual(restarts(), [1, 1, 0, 0, 0, 0, 0, 0]);
	pitched.advance(burst.slice(0, 1), 0, 10, 1, -1, () => listener);
	assert.deepEqual(restarts(), [2, 1, 0, 0, 0, 0, 0, 0], "the ninth trigger restarts the oldest voice");
	pitched.advance(burst.slice(0, 2), 0, 10, 1, -1, () => listener);
	assert.deepEqual(restarts(), [2, 2, 1, 0, 0, 0, 0, 0], "then the next oldest, in turn");
